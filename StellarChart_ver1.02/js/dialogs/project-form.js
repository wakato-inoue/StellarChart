...
function handleFormSubmit(e) {
  try {
    ...
    let progress = parseInt(formData.get('progress') || '0', 10);
    ...
    const confirmMsg = `以下の内容で${editingProjectId !== null ? '更新' : '作成'}します。よろしいですか？\n\nプロジェクト名：${name}\n概要：${description.length > 50 ? description.slice(0, 50) + '...' : description}\n期間：${startDate} 〜 ${endDate}\nステータス：${getStatusLabel(status)}\n予定工数：${plannedHours}h`;
    ...
    showConfirmDialog(confirmMsg, () => {
      ...
      showCompleteDialog(`${editingProjectId !== null ? '更新' : '登録'}が完了しました。`, () => {
        projectDialog.close();
      });
    });
  } catch (err) {
