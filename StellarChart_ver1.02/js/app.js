...
function initApp() {
  ...
  Object.entries(projFieldMap).forEach(([field, id]) => {
  ...
  taskFilterStatus.addEventListener('change', () => {
    if (taskFilterStatus.value === 'completed') {
      taskFilterDone.checked = true;
    } else if (taskFilterDone.checked) {
      taskFilterDone.checked = false;
    }
    renderTasks();
  });
  taskFilterPriority.addEventListener('change', renderTasks);
  taskFilterDone.addEventListener('change', renderTasks);
  ...
  // 依頼パネル制御
  btnIssues.addEventListener('click', (e) => {
    e.preventDefault();
    openIssuePanel();
  });
  ...
  // 転送ダイアログ制御
  btnCloseTransfer.addEventListener('click', cancelTransfer);
  ...
  // Escキーによる転送キャンセル時の状態リセット
  transferDialog.addEventListener('close', () => {
    delete transferDialog.dataset.mode;
    delete transferDialog.dataset.taskId;
    const wasCompleted = transferDialog.dataset.completed === '1';
    delete transferDialog.dataset.completed;
    if (!wasCompleted && currentDetailTaskId) {
      renderTaskDetail(currentDetailTaskId);
    }
  });
