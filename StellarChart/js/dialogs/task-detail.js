...
//   openTaskDetail(taskId)          - タスク詳細モーダルを開く（全情報HTML生成・イベントバインド）
...
function openTaskDetail(taskId) {
  ...
  if (_canChangeStatus) {
    ...
    <div class="detail-section detail-edit-section">
      ...
      <div class="detail-btn-row">
        ...
        ${task.status === 'needs_action' && _canTransfer ? `<button id="btn-detail-transfer" class="btn btn-secondary">転送する</button>` : ''}
      </div>
    </div>`;
  }
  ...
  // 転送ボタン
  const btnTransfer = document.getElementById('btn-detail-transfer');
  if (btnTransfer) {
  ...
  document.querySelectorAll('.btn-effort-log-delete').forEach(btn => {
    btn.addEventListener('click', () => {
      if (!confirm('この作業履歴を削除してもよろしいですか？')) return;
      deleteEffortLogEntry(btn.dataset.taskId, btn.dataset.logId);
    });
  });
}
...
function updateTaskFromDetail(taskId) {
  ...
  const threadInput = document.getElementById('thread-input');
  if (newStatus === 'needs_action' && task.status !== 'needs_action') {
    const comment = threadInput ? threadInput.value.trim() : '';
    if (!comment) {
      alert('要対応に変更する場合はコメントを入力してください。');
      openTaskDetail(taskId);
      return;
    }
  ...
  if (prevStatus !== newStatus) {
    addStatusHistory(task, prevStatus, newStatus, '');
  }
