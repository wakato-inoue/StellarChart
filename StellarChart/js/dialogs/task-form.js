...
function openEditTaskDialog(taskId) {
  ...
  taskActHoursInput.value = task.actualHours || 0;
  taskProgressInput.value = task.progress || 0;
  taskProgressValDisplay.textContent = `${task.progress || 0}%`;

  if (formGroupStatus) formGroupStatus.style.display = '';
  if (formGroupActualHours) formGroupActualHours.style.display = '';
  if (formGroupProgress) formGroupProgress.style.display = '';
  taskDialog.showModal();
  taskDialog.scrollTop = 0;
}
